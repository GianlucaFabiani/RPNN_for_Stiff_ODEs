...
t0=0; %initial time
tf=15; %final time
u0=[1;-6;1;-6;-10.60660171779820]; %initial conditions
%
f=@(t,x) ex11ode(t,x); %ODE system
derf=@(t,x) ex11ode_jac(t,x); %Jacobian (For RPNN the Jacobian need to be transposed)
%
nEq=max(size(u0));
%Mass Matrix
Mmass=diag([1,1,1,1,0]);
%
nEq=max(size(u0)); %number of equations
var_name={'u_1','u_2','u_3','u_4','u_5'}; %name of variables
%
